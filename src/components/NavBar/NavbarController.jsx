...
export default function NavbarController({ hasNavigation, showModal }) {
  ...
  const NavBarStyle =
    "responsive:flex responsive:text-white responsive:divide-x responsive:divide-button";
  const NavBarScrolledStyle =
    "responsive:flex responsive:items-center responsive:text-white responsive:divide-x responsive:divide-button";
  const mobileNavBarStyle =
    "fixed bg-primary z-[60] h-full w-full left-0 text-white top-[66px] transition-transform duration-300 ease-in-out";
  ...
  return (
    <>
      {isLoginFormOpen && <LogInForms toggleLoginForm={setLoginFormState} />}
      ...
      {isMobileView && (
        <NavBar
          ...
          style={
            mobileNavBarStyle +
            (isBurguerMenuOpen ? "" : mobileNavBarStyleClose)
          }
          isActivated={isActivated}
        />
      )}
